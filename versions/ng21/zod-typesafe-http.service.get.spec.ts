...
describe('TypesafeHttpService', () => {
  ...
  describe('get(...) method', () => {
    ...
    describe('simple call-through functionality', () => {
      it('should return the same value as http.get', async () => {
        // given
        mockHttpClient.get.mockReturnValue(of(SAMPLE_VEHICLE_TITLE));

        // when
        const result = await firstValueFrom(service.get(API_URL, { ...HTTP_OPTIONS, responseType: 'text'}));

        // then
        expect(result).toEqual(SAMPLE_VEHICLE_TITLE);
      });

      it('should propagate error from http.get', async () => {
        // given
        ...
        // when
        try {
          await firstValueFrom(service.get(API_URL, { ...HTTP_OPTIONS, responseType: 'text'}));
        } catch (error) {
          // then
    ...
    describe('response validation', () => {
      it('should accept valid response', async () => {
        ...
        // when
        const result = await firstValueFrom(service.get<VehicleType>(API_URL, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
      ...
      it('should reject invalid response', async () => {
        ...
        // when
        try {
          await firstValueFrom(service.get<VehicleType>(API_URL, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
        } catch (error) {
          // then
