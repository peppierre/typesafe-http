  ...
  describe('post(...) method', () => {
    describe('foundational functionality', () => {
      it('should call http.post once', () => {
        ...
        // when
        service.post(API_URL, null, HTTP_OPTIONS);
    ...
    describe('simple call-through functionality', () => {
      it('should return the same value as http.post', async () => {
        // given
        mockHttpClient.post.mockReturnValue(of(SAMPLE_VEHICLE_TITLE));

        // when
        const result = await firstValueFrom(service.post(API_URL, null, { ...HTTP_OPTIONS, responseType: 'text'}));

        // then
        expect(result).toEqual(SAMPLE_VEHICLE_TITLE);
      });

      it('should propagate error from http.post', async () => {
        // given
        ...
        // when
        try {
          await firstValueFrom(service.post(API_URL, null, { ...HTTP_OPTIONS, responseType: 'text'}));
        } catch (error) {
          // then
    ...
    describe('response validation', () => {
      it('should accept valid response', async () => {
        ...
        // when
        const result = await firstValueFrom(service.post<VehicleType>(API_URL, null, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
      ...
      it('should reject invalid response', async () => {
        ...
        // when
        try {
          await firstValueFrom(service.post<VehicleType>(API_URL, null, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
        } catch (error) {
          // then
