...
describe('TypesafeHttpService', () => {
  ...
  describe('put(...) method', () => {
    ...
    describe('simple call-through functionality', () => {
      it('should return the same value as http.put', async () => {
        // given
        mockHttpClient.put.mockReturnValue(of(SAMPLE_VEHICLE_TITLE));

        // when
        const result = await firstValueFrom(service.put(API_URL, null, { ...HTTP_OPTIONS, responseType: 'text'}));

        // then
        expect(result).toEqual(SAMPLE_VEHICLE_TITLE);
      });

      it('should propagate error from http.put', async () => {
        // given
        ...
        // when
        try {
          await firstValueFrom(service.put(API_URL, null, { ...HTTP_OPTIONS, responseType: 'text'}));
        } catch (error) {
          // then
    ...
    describe('response validation', () => {
      it('should accept valid response', async () => {
        ...
        // when
        const result = await firstValueFrom(service.put<VehicleType>(API_URL, null, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
      ...
      it('should reject invalid response', async () => {
        ...
        // when
        try {
          await firstValueFrom(service.put<VehicleType>(API_URL, null, { ...HTTP_OPTIONS, runtimeType: VEHICLE }));
        } catch (error) {
          // then
