...
describe('TypesafeHttpService', () => {
  ...
  describe('options(...) method', () => {
    ...
    describe('simple call-through functionality', () => {
      it('should return the same value as http.options', (done) => {
        // given
        mockHttpClient.options.mockReturnValue(of(SAMPLE_VEHICLE_TITLE));

        // when
        service.options(API_URL, { ...HTTP_OPTIONS, responseType: 'text'}).subscribe((response) => {
          // then
      ...
      it('should propagate error from http.options', (done) => {
        // given
        ...
        // when
        service.options(API_URL, { ...HTTP_OPTIONS, responseType: 'text'}).subscribe({
          next: () => {
            // then
    ...
    describe('response validation', () => {
      it('should accept valid response', (done) => {
        ...
        // when
        service
          .options<VehicleType>(API_URL, { ...HTTP_OPTIONS, runtimeType: VEHICLE })
          .subscribe((response) => {
            // then
      ...
      it('should reject invalid response', () => {
        ...
        // when
        try {
          service.options<VehicleType>(API_URL, { ...HTTP_OPTIONS, runtimeType: VEHICLE }).subscribe();
        } catch (error) {
          // then
